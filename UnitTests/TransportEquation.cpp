...
class TransportEquationTestsWithAdiOpeator: public TransportEquationTests
{
};

TEST_F(TransportEquationTestsWithAdiOpeator, ZeroTransportCoefficients)
{
...
TEST_F(TransportEquationTestsWithAdiOpeator, ConstantSolution)
{
	constexpr auto solver = pde::SolverType::ADI;
	// test single precision
	{
		...
		for (size_t n = 0; n < 10; ++n)
		{
			...
			for (std::size_t k = 0; k < nSpacePoints[2]; ++k)
				for (std::size_t i = 0; i < nSpacePoints[0]; ++i)
					for (std::size_t j = 0; j < nSpacePoints[1]; ++j)
						// don't use float, that's a terrible numerical accuracy
						ASSERT_NEAR(solution[GetIndex(i, j, k)], inputDataFloat.initialCondition[GetIndex(i, j, k)], 0.03f) << static_cast<int>(solver);
		}
	}
	...
	// test double precision
	{
		...
		for (size_t n = 0; n < 10; ++n)
		{
			...
			for (std::size_t k = 0; k < nSpacePoints[2]; ++k)
				for (std::size_t i = 0; i < nSpacePoints[0]; ++i)
					for (std::size_t j = 0; j < nSpacePoints[1]; ++j)
						EXPECT_NEAR(solution[GetIndex(i, j, k)], inputDataDouble.initialCondition[GetIndex(i, j, k)], 6.08e-11) << static_cast<int>(solver);
		}
	}
}

TEST_F(TransportEquationTestsWithAdiOpeator, TransportAlongAxes)
{
	constexpr auto solver = pde::SolverType::ADI;
	// test single precision
//	{
					...
					for (std::size_t i = 1; i < nSpacePoints[0] - 1; ++i)
					{
						for (std::size_t j = 1; j < nSpacePoints[1] - 1; ++j)
						{
							...
							if (axisIdx == 0)
							{
								...
								const auto expected = InitialCondition(characteristicX, inputDataDouble.spaceGrids[1][j], inputDataDouble.spaceGrids[2][k]);
								const auto actual = solution[GetIndex(i, j, k)];
								EXPECT_NEAR(actual, expected, 0.0125) << n << " | " << i << " | " << j << " | " << k;
							}
							else if (axisIdx == 1)
							{
								const auto characteristicY = inputDataDouble.spaceGrids[1][j] + velocity * inputDataDouble.deltaTime * static_cast<double>(n + 1);
								const auto expected = InitialCondition(inputDataDouble.spaceGrids[0][i], characteristicY, inputDataDouble.spaceGrids[2][k]);
								const auto actual = solution[GetIndex(i, j, k)];
								ASSERT_NEAR(actual, expected, 0.012) << n << " | " << i << " | " << j << " | " << k;
							}
							else if (axisIdx == 2)
							{
								const auto characteristicZ = inputDataDouble.spaceGrids[2][k] + velocity * inputDataDouble.deltaTime * static_cast<double>(n + 1);
